...
A portable, two-module Databricks workshop (~55 min each) covering Classical ML and GenAI/Agents. Notebooks are Databricks-format Python files (header `# Databricks notebook source`, cells separated by `# COMMAND ----------`). The workshop must run on any Databricks workspace — all catalog/schema/endpoint references come from `_resources/00_config.py`.
...
- **Module 1 (`module_1_classical_ml/`)** — Sequential notebooks 01-07: data exploration → feature engineering (Feature Store + UC) → model training (LightGBM + Optuna) → UC model registry → model serving → batch inference → Lakehouse Monitoring. Notebook `03a` is a markdown-only alternative track for Genie Code.
- **Module 2 (`module_2_genai/`)** — Sequential notebooks 01-07: AI Functions (ai_query/ai_extract) → UC tool creation → LangGraph agent + MCP → MLflow Tracing → agent evaluation → Databricks Apps deployment → AI Gateway.
